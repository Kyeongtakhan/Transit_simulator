...
*/
#define CONSTPASS 1
std::random_device deterministic_dev;
std::mt19937 deterministic_e(deterministic_dev());
std::uniform_int_distribution<std::mt19937::result_type> deterministic_dist(1,
   1000);
...
Passenger * DeterministicPassengerFactory::Generate(int curr_stop,
   int last_stop) {
...
#ifndef CONSTPASS
  int destination = (deterministic_dist(deterministic_e) %
  (last_stop - curr_stop)) + curr_stop + 1;
#endif
...
/*
  This is the function that uses macros not to generates different passenger.
  This function only used for regression testing
*/
std::string DeterministicPassengerFactory::NameGeneration() {
  // assume rand is seeded
#ifndef CONSTPASS
  std::string name = std::string(
    NamePrefixArray[(deterministic_dist(deterministic_e) % 7)]) +
    std::string(
      NameStemsArray[(deterministic_dist(deterministic_e) % 20)]) +
    std::string(
      NameSuffixArray[(deterministic_dist(deterministic_e) % 16)]);
#endif
