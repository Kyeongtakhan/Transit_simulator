...
/*
  This is a Regression configuration manager constructor that initiates the ConfigManager object.
*/
RtestConfigManager::RtestConfigManager() : routes_(std::vector<Route *>()) {
}
/*
  This is a Regression configuration manager destructor that destructs the routes and elements in the vector
*/
RtestConfigManager::~RtestConfigManager() {
...
/*
  This is the function that reads the files from the user want and
  generates the routes from the text files. I add one more functionality that converts the last probability from something to zero.
  If the probability of last stop is not zero, the program will throw the error. Also, theoretically, it is not possible for passenger to come to last station.
  ReadConfig function will automatically change last stop probability to zero. The difference between original config_manager is that this will uses regression test passenger generator when making the routes.
*/
void RtestConfigManager::ReadConfig(const std::string filename) {
    ...
    while (std::getline(configFile, line)) {
        ...
        if (chunk == "ROUTE_GENERAL") {
            ...
            if (stops.size() > 0) {
                ...
                /*
                Rregression Passenger generator is passed to new Route constructor to have deterministic passenger generation.
                */
                RtestPassengerGenerator * generator
                = new RtestPassengerGenerator(currProbabilities, stops);
                routes_.push_back(new Route(
                  currGeneralName + " " + currRouteName,
                 rawStops, rawDists,
                  static_cast<int>(stops.size()), generator));
                stops.clear();
                distances.clear();
            ...
            currRouteName.erase(std::remove(
              currRouteName.begin(),
            ...
            stopName.erase(std::remove(
              stopName.begin(), stopName.end(), ' '), stopName.end());

            // Check if the stop already exists
            std::vector<std::string>::iterator it
            = std::find(stopNames.begin(), stopNames.end(), stopName);
            if (it != stopNames.end()) {
            ...
            stops.push_back(new Stop(id,
               std::stod(latString), std::stod(lonString)));
            ...
            if (stops.size() > 1) {
                double dist = sqrt(
                  (latitude-oldLat)*(latitude-oldLat) +
