...
int main() {
  ...
  */
  ...
  std::cout  << std::endl << "\t*** Stop Reports ***" <<
   std::endl << std::endl;
  std::cout  << std::endl << "\t\t ~ Eastbound ~ " <<
   std::endl << std::endl;
  ...
  for (int i = 0; i < rounds; i++) {
    ...
    if (campus_connector1.IsTripComplete()) {
      ...
      campus_connector1 = Bus(std::to_string(cc1_counter),
       &CC1_EB, &CC1_WB, 60, 1);
    }
    if (campus_connector2.IsTripComplete()) {
      cc2_counter++;
      CC2_EB = Route("Campus Connector 1- Eastbound",
       CC_EB_stops, CC_EB_distances, 6, &CC_EB_generator);
      CC2_WB = Route("Campus Connector 1- Westbound",
       CC_WB_stops, CC_WB_distances, 6, &CC_WB_generator);
      campus_connector1 = Bus(std::to_string(cc2_counter),
       &CC2_EB, &CC2_WB, 60, 1);
    }
